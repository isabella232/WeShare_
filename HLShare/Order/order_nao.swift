...
class EditOrderNao : BaseNao<Result> {
	override init() {
		super.init()
		baseUrl = Config.BASE_URL+"lease/"
	}
...
////debug only
//class DeleteOrderNao : BaseEntityNao<Result> {
//    override init() {
//        super.init()
//        baseUrl = Config.BASE_URL+"lease/"
//    }
...
//    override func getDeleteQuerier<DeleteOrderResult>(_ id: Int) -> Querier<Result> {
...
class OrderListNao : BaseNao<ListLeaseOrdersResult> {
    ...
    override func parseJson(_ json: String) -> Any? {
        return ListLeaseOrdersResult.deserialize(from: json)
    }
	...
	static func getListAsBuyerQuerier() -> Querier<ListLeaseOrdersResult> {
		// TODO
        var q = Querier<ListLeaseOrdersResult>("lessee/order", nil, nil)
        q.params["state"] = 0
		return q
	}
	/** 作为卖家的订单列表 */
