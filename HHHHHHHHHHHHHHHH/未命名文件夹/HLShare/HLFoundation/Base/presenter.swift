...
class Presenter<R> : PPresenter {
    ...
    /**  */
    var querierProcessor : IQuerierProcessor<R>? = nil
	/** 默认的查询器 */
	...
	* @return 错误码（表示同步结果）
	*/
	func execute<T>(_ nao : Nao<T> , _ querier : Querier<T> , _ fnOnResponse : FnOnResponse<T>? , _ fnOnError : FnOnError<T>?, _ processor : IQuerierProcessor<T>? = nil) -> Int {
	...
	*/
	func execute() -> Int {
		if nao == nil || querier == nil { return ErrorCode.FAILURE}
		if let l = listener {return execute(nao!, querier!, l, querierProcessor)}
		return execute(nao!, querier!, fnOnResponse, fnOnError, querierProcessor)
	}
}
...
class OperationPresenter<O> : EntityPresenter<O> {
	...
	*/
	func operate(_ targetId : Int, _ listener : IListener<O>) -> Int {
		return execute(nao!, operationNao!.getOperateQuerier(targetId), listener, querierProcessor);
	}
}
...
/** 评价Presenter */
class ReviewPresenter<R> : OperationPresenter<R> {
	override init() {
		super.init()
		pid = Business.OP_REVIEW
	}
    //override var querier: Querier<F>? {}
    func review(_ targetId : Int, _ role : Int, _ score : Int, _ content : String?, _ listener : IListener<R>) -> Int {
        var reviewNao = nao as! ReviewNao<R>
        return execute(nao!, reviewNao.getReviewQuerier(targetId, role, score, content), listener, querierProcessor)
    }
}

class EditPresenter<E> : EntityPresenter<E> {
    override init() {
        super.init()
        pid = Business.OP_MODIFY
    }
}
