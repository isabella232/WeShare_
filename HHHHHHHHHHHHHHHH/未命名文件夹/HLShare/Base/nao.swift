...
class Nao<R> : PJsonParser {
    ...
    func parseJson(_ json: String) -> Any? {
        return Result.deserialize(from: json)
    }
...
class OperationNao<T> : BaseEntityNao<T> , POperationNao {
    ...
    static func processInputQuerier<T>(_ targetId: Int, _ querier : Querier<T> ) {
        querier.params["targetId"] = targetId
        //return querier
    }
}
...
class ReviewNao<T> : OperationNao<T> {
    ...
    func getReviewQuerier(_ targetId : Int, _ role : Int, _ score : Int, _ content : String?) -> Querier<T> {
        let querier = getOperateQuerier(targetId)
        querier.params["score"] = score
        querier.params["content"] = content
        querier.params["role"] = role
        return querier
    }
}
